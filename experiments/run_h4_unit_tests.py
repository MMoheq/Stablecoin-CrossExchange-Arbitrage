...
def test_kickback_risk_zero_when_no_remaining_time(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(10.0))
    assert h4.estimate_chain_kickback_risk_score("binance", "USDT", 0.0) == 0.0
    assert h4.estimate_chain_kickback_risk_score("binance", "USDT", -5.0) == 0.0


def test_kickback_risk_zero_when_fastest_hop_slower_than_window(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(30.0))
    assert h4.estimate_chain_kickback_risk_score("binance", "USDT", 20.0) == 0.0


def test_kickback_risk_between_zero_and_one(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(10.0))
    score = h4.estimate_chain_kickback_risk_score("binance", "USDT", 20.0)
    assert score == pytest.approx(0.5, rel=1e-6)


def test_kickback_risk_clamped_to_one(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(0.0001))
    score = h4.estimate_chain_kickback_risk_score("binance", "USDT", 10_000.0)
    assert 0.0 <= score <= 1.0


def test_kickback_risk_zero_when_no_transfer_info(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", lambda n: None)
    assert h4.estimate_chain_kickback_risk_score("binance", "USDT", 60.0) == 0.0


def test_chain_congestion_heuristic_uses_weight(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(10.0))
    old_weight = h4.CHAIN_HEURISTIC_WEIGHT
    h4.CHAIN_HEURISTIC_WEIGHT = 2.0
    try:
        assert h4.chain_congestion_heuristic_cost("binance", "USDT", 20.0) == pytest.approx(1.0)
    finally:
        h4.CHAIN_HEURISTIC_WEIGHT = old_weight


def test_chain_congestion_heuristic_non_negative(monkeypatch):
    monkeypatch.setattr(h4, "_fastest_transfer_time_for_node", _fake_fastest(10.0))
    assert h4.chain_congestion_heuristic_cost("binance", "USDT", 20.0) >= 0.0


def test_estimate_exchange_reliability_score_known_exchange():
    assert h4.estimate_exchange_reliability_score("binance") == pytest.approx(0.9)


def test_estimate_exchange_reliability_score_unknown_exchange():
    assert h4.estimate_exchange_reliability_score("some_weird_exchange") == 0.0


def test_exchange_risk_heuristic_cost_known_exchange():
    assert h4.exchange_risk_heuristic_cost("binance") == pytest.approx(0.1)


def test_exchange_risk_heuristic_cost_unknown_exchange_uses_fallback():
    assert h4.exchange_risk_heuristic_cost("unknown_exch") == h4.UNKNOWN_EXCHANGE_PENALTY


def test_exchange_risk_respects_weight():
    old_weight = h4.EXCHANGE_HEURISTIC_WEIGHT
    h4.EXCHANGE_HEURISTIC_WEIGHT = 2.0
    try:
        expected = (1 - 0.6) * 2.0  # kucoin = 0.6
        assert h4.exchange_risk_heuristic_cost("kucoin") == pytest.approx(expected)
    finally:
        h4.EXCHANGE_HEURISTIC_WEIGHT = old_weight
